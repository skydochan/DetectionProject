...
void YOLOV7_face::nms(vector<FaceBoxInfo>& input_boxes)
{
	sort(input_boxes.begin(), input_boxes.end(), [](FaceBoxInfo a, FaceBoxInfo b) { return a.score > b.score; });
	...
	// return post_nms;
	int idx_t = 0;
	input_boxes.erase(remove_if(input_boxes.begin(), input_boxes.end(), [&idx_t, &isSuppressed](const FaceBoxInfo& f) { return isSuppressed[idx_t++]; }), input_boxes.end());
}

int YOLOV7_face::detect(Mat& frame)
{
	Mat dstimg;
	...
	vector<FaceBoxInfo> generate_boxes;
	...
	for (n = 0; n < this->num_proposal; n++)   ///特征图尺度
	{
		...
		if (box_score > this->confThreshold)
		{
			...
			if (class_socre > this->confThreshold)
			{
				...
				generate_boxes.push_back(FaceBoxInfo{ xmin, ymin, xmax, ymax, class_socre, kpt1,kpt2,kpt3,kpt4,kpt5 });
			}
		}
		pdata += nout;
	}
